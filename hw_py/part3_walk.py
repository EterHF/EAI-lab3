...
class MyWalkEnv(Joystick):
    def step(self, state: mjx_env.State, action: jax.Array):
        ...
        lin_vel_error_x = jp.square(body_lin_vel[0] - DESIRED_XY_LIN_VEL[0])
        lin_vel_error_y = jp.square(body_lin_vel[1] - DESIRED_XY_LIN_VEL[1])
        
        tracking_sigma = 0.25
        tracking_lin_vel = jp.exp(-0.5 * (lin_vel_error_x + lin_vel_error_y) / (tracking_sigma**2))
        
        ang_vel_error = jp.square(body_ang_vel[2] - DESIRED_YAW_ANG_VEL)
        tracking_ang_vel = jp.exp(-0.5 * ang_vel_error / (tracking_sigma**2))
...
def train_ppo():
    ...
    os.makedirs('./experiments/solutions', exist_ok=True)
